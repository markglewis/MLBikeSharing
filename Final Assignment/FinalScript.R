...
#Is there a strong correlation between Month and Season?
#Plot the scatter plot between Month and Season
plot(bike$mnth,bike$season, col="lightskyblue4",xlab="Month",ylab="Season",main = "Correlation between Months and Seasons")
##MARK Need your help here to draw the line of best fit###
#USe the X-squared test to check the significance of the correlation between the two variables
chisq.test(bike$mnth, bike$season, correct = FALSE)
#Since the p-value is < 2.2e-16 is less than 0.05, we can reject the null hypothesis in favor of alternative hypothesis and conclude that Month and Season are dependent on each other. 
...
#data preperation gets rid of unecessary features
data$dteday <- NULL
data$instant<- NULL
...
kfullresult$size
plot(data$cnt, kfullresult$cluster)

#now without registrations
data$dteday <- NULL
data$instant<- NULL
data$casual <- NULL
...
kresult$size
#clusters based on the hour of day
#cnt registered and casual are the biggest factors in the groupings
#probably due ot their variance

#hclustering produces to many clusters
#hclustresult <- hclust(dist(data),method = "average")
#summary(hclustresult)
#plot(hclustresult)
#hclustresult <- hclust(dist(data),method = "single")
#summary(hclustresult)
#plot(hclustresult)
#hclustresult <- hclust(dist(data),method = "complete")
#summary(hclustresult)
#plot(hclustresult)
#abline(h = 200, col = "red")
#cutree(hclustresult, h = 7)

#dbscanresult<- dbscan(data, eps =.15, MinPts = 5)
#summary(dbscanresult$cluster)
...
table(data$cnt,kTimeresult$cluster)
...
plot(data$cnt, data$weathersit, col=kweatherresult$cluster)
...
pca <- prcomp(data)
cor(data)
#data wasnt highlt correlated
#no justification for principle component analysis
#did it anyway cause why not
print(pca)
summary(pca)
abs(pca$rotation)
plot(pca, type="lines")
...
#building the model
#all the following models have k fold cross validation
#random forest model
data <- read.csv("hour.csv")
set.seed(123)
...
#lets evaluate the models we have so far
...
#now integrate clusters into data
...
train_control<- trainControl(method="cv", number=10, verboseIter = TRUE,importance=T)
